...
class BNN:
        ...
        with tf.variable_scope(self.name):
            ...
            if not self.deterministic:
                ...
                if self.source == 'd4rl':
                    ...
                    train_loss += 0.01 * tf.reduce_mean(self.max_logvar) - 0.01 * tf.reduce_mean(self.min_logvar)
                else:
    ...
    def save(self, savedir, timestep):
        ...
        name_prefix = self.name # '{}_{}'.format(self.name, timestep)
    ...
    def _compile_losses(self, inputs, targets, inc_var_loss=True):
        ...
        if inc_var_loss:
            if self.source == 'neorl':
                dist = tf.contrib.distributions.Normal(loc=mean, scale=tf.sqrt(tf.exp(log_var)))
                total_losses = tf.reduce_mean(- dist.log_prob(targets))  # mse_losses + var_losses
