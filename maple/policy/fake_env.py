...
class FakeEnv:
    ...
    def reset(self, rollout_length, batch_size):

        self.ts = 0
        # Randomly select 10 models rather than using one model for adaptable policy learning.
        # Then MAPLE learns to adapt for each combination of the dynamics models.
        ensemble_number = np.random.randint(10, len(self.model._model_inds))
        sub_model_idx = np.random.choice(self.model._model_inds, size=ensemble_number)
        self.model_inds = np.random.choice(sub_model_idx, size=batch_size)
        self.rollout_length = rollout_length
        self.reset_ratio = np.random.random()
    ...
    def step(self, obs, act, deterministic=False):
        ...
        if self.penalty_coeff != 0:
            ...
            penalty = np.clip(penalty, a_max=self.max_penalty, a_min=None)
